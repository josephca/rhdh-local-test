...
set_cluster_info() {
  export K8S_CLUSTER_URL=$(cat ${VAULT_SECRETS_DIR}/RHDH_PR_OS_CLUSTER_URL)
  export K8S_CLUSTER_TOKEN=$(cat ${VAULT_SECRETS_DIR}/RHDH_PR_OS_CLUSTER_TOKEN)
  
  if [[ "$JOB_NAME" == *ocp-v4-14 ]]; then
    K8S_CLUSTER_URL=$(cat ${VAULT_SECRETS_DIR}/RHDH_OS_1_CLUSTER_URL)
    K8S_CLUSTER_TOKEN=$(cat ${VAULT_SECRETS_DIR}/RHDH_OS_1_CLUSTER_TOKEN)
  elif [[ "$JOB_NAME" == *ocp-v4-13 ]]; then
    K8S_CLUSTER_URL=$(cat ${VAULT_SECRETS_DIR}/RHDH_OS_2_CLUSTER_URL)
    K8S_CLUSTER_TOKEN=$(cat ${VAULT_SECRETS_DIR}/RHDH_OS_2_CLUSTER_TOKEN)
  elif [[ "$JOB_NAME" == *aks* ]]; then
    K8S_CLUSTER_URL=$(cat ${VAULT_SECRETS_DIR}/RHDH_AKS_CLUSTER_URL)
    K8S_CLUSTER_TOKEN=$(cat ${VAULT_SECRETS_DIR}/RHDH_AKS_CLUSTER_TOKEN)
  fi
}
...
configure_external_postgres_db() {
  ...
  POSTGRES_HOST=$(printf "%s" "postgress-external-db-primary.$NAME_SPACE_POSTGRES_DB.svc.cluster.local" | base64 | tr -d '\n')  
...
apply_yaml_files() {
  ...
  if [[ "$JOB_NAME" == *aks* || "$JOB_NAME" == *gke* || "$JOB_NAME" == *operator* ]]; then
    ...
    GITHUB_APP_APP_ID=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_4_APP_ID)
    GITHUB_APP_CLIENT_ID=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_4_CLIENT_ID)
    GITHUB_APP_PRIVATE_KEY=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_4_PRIVATE_KEY)
    GITHUB_APP_CLIENT_SECRET=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_4_CLIENT_SECRET)
  else 
    echo "[INFO]: Overriding to use JANUS_TEST GITHUB_APP for local test"
    GITHUB_APP_APP_ID=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_JANUS_TEST_APP_ID)
    GITHUB_APP_CLIENT_ID=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_JANUS_TEST_CLIENT_ID)
    GITHUB_APP_PRIVATE_KEY=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_JANUS_TEST_PRIVATE_KEY)
    GITHUB_APP_CLIENT_SECRET=$(cat ${VAULT_SECRETS_DIR}/GITHUB_APP_JANUS_TEST_CLIENT_SECRET)
  fi
...
initiate_deployments() {
  ...
  if [[ "$1" == "1" || "$1" == "11" || "$1" == "2" || "$1" == "21" ]]; then
    ...
    echo "-------------------------------------------------------------"
    echo "Deploying image from repository: ${QUAY_REPO}:${TAG_NAME}, in NAME_SPACE : ${NAME_SPACE}"
    echo "-------------------------------------------------------------"
...
main() {
  ...
  source "${DIR}/local_env_variables_1.4.sh"
  ...
  install_oc
  echo "CLUSTER_TOKEN : ${K8S_CLUSTER_TOKEN}  URL: ${K8S_CLUSTER_URL}"
  if [[ "$JOB_NAME" == *aks* ]]; then
    az_login
    ...
    gcloud_auth "${GKE_SERVICE_ACCOUNT_NAME}" "${VAULT_SECRETS_DIR}/GKE_SERVICE_ACCOUNT_KEY"
...
# When you run ./localtest.sh, it displays options to run
show_menu() {
  echo "Select an option:"
  echo "1)  [install + test]: install (RHDH, RHDH_RBAC) and run RHDH e2e tests with 'janus-test' GH App"
  echo "11) [install + test]: install RHDH and run e2e tests with 'janus-test' GH App"
  echo "12) [install + test]: install RHDH_RBAC and run e2e tests with 'janus-test' GH App"
  echo "-------------------------------------------------------------------------"
  echo "111) [test] run RHDH e2e tests only (if RHDH is already installed) with 'janus-test' GH App"
  echo "121) [test] run RHDH_RBAC e2e tests only (if RHDH_RBAC is already installed) with 'janus-test' GH App"
  echo "-------------------------------------------------------------------------"
...
run_option() {
    case $1 in
        1)
            echo "[install + test]: install (RHDH, RHDH_RBAC) and run RHDH e2e tests with 'janus-test' GH App"
            _GITHUB_APP_ORG="janus-test"
            main $1
            ;;
        11)
            echo "[install + test]: install RHDH and run e2e tests with 'janus-test' GH App"
            _GITHUB_APP_ORG="janus-test"
            main $1
            ;;
        111)
            echo "[test] run RHDH e2e tests only (if RHDH is already installed) with 'janus-test' GH App"
            _GITHUB_APP_ORG="janus-test"
            main $1
            ;;
        12)
            echo "[install + test]: install RHDH_RBAC and run e2e tests with 'janus-test' GH App"
            _GITHUB_APP_ORG="janus-test"
            main $1
            ;;
        121)
            echo "[test] run RHDH_RBAC e2e tests only (if RHDH_RBAC is already installed) with 'janus-test' GH App"
            _GITHUB_APP_ORG="janus-test"
            main $1
            ;;
        *)
